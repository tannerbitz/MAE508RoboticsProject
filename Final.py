...
if __name__ == '__main__':
	...
	okay, imageN = capture.read()
	    ...
	    locB = track(imageN,blue_lower,blue_upper)
	    locG = track(imageN,green_lower,green_upper)
            diff = ( locB[0] - locG[0], locB[1] - locG[1])
	    frontBot = locB
	    center   = ((locB[0]+locG[0])/2,(locB[1]+locG[1])/2)
	    closestPt= refPtAll[0]
	    if len(refPtAll) < 2:
		closestPt2 = closestPt
	    else:
	        closestPt2 = refPtAll[1]
	    diff2= (center[0]-closestPt[0], center[1]-closestPt[1])
	    diff3= (center[0]-closestPt2[0], center[1]-closestPt2[1])
	    ...
	    dist = ((center[0]-closestPt[0])**2+(center[1]-closestPt[1])**2)**.5
	    if dist<35:
		refPtAll.pop(0)
		if len(refPtAll) == 0:
		   sendAddressStop(IP)
		   cv2.imwrite( 'RecentTest.png', image)
		   break
	    cv2.circle(image,center,4,(0,100,0),10)
            botAngle = np.angle(diff[0]+diff[1]*1j, deg=True)
	    nearAngle= np.angle(diff2[0]+diff2[1]*1j, deg=True)
            nearAngle2=np.angle(diff3[0]+diff3[1]*1j, deg=True)
	    ...
	    refAngle2 = botAngle-nearAngle2
	    if refAngle2 < -180:
                refAngle2 = refAngle2 + 360
            if refAngle2 > 180:
                refAngle2 = refAngle2 - 360
            if abs(refAngle2) > 100:
                refAngle = 100*np.sign(refAngle2)
	    #if abs(refAngle) > 80:
		#kP = kPh
		...
		kD    = 0
	    constFactor = 25
	    refAngleIn = dist/(dist+constFactor)*refAngle + constFactor/(dist+constFactor)*refAngle2
	    sendAddressData(refAngleIn,IP,lVolt,rVolt,kP,kI,kD)#refAngle,IP)
	    #print(refAngle)
