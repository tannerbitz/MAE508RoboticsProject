...
//  Serial.print(angle);
//  Serial.print("    ");
//  Serial.print(leftFullVolt);
//  Serial.print("    ");
//  Serial.print(rightFullVolt);
//  Serial.print("    ");
//  Serial.print(kp);
//  Serial.print("    ");
//  Serial.println(ki);
...
void isrFunction(){
  ...
  if (ki==0.0){             // Check for ki.  If ki == 0, then code for P controller
    e_new = -angle;
    ov_new = kp*e_new;
  }
  else{                     // If ki is non-zero, then use PI controller
    e_old = e_new;
    e_new = -angle;
    ov_old = ov_new;
    ov_new = ov_new + (kp+ki)*e_new - kp*e_old;    
  }
